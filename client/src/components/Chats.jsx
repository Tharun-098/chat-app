  ...
  return (
    <div className='h-screen flex flex-col'>
      <div className='flex items-center gap-2 border-b-1 border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 p-2'>
        <ArrowLeft onClick={()=>{navigate('/');setChat(false);setSelectUser(null);localStorage.removeItem('selectuser')}} className="dark:text-gray-400 text-gray-600" />
        <img src={selectUser.profilePicture || userIcon} className='w-15 h-15 rounded-full'/>
        <div>
        <h1 className='text-md font-semibold dark:text-white'>{selectUser.username}</h1>
  ...
  <div key={index}>
    <div className={`flex my-2 ${mess.senderid === userr._id ? 'items-end' : 'items-start'} flex-col`}>
      ...
      <div className='p-3 relative dark:bg-gray-950'>
        <input onKeyDown={(e)=>e.key=="Enter"?handleSendMessage(e):null} type="text" placeholder='message' value={input} onChange={(e)=>setInput(e.target.value)} className='pl-15 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-200 w-full outline-0 border border-gray-400 rounded-lg bg-gray-200 p-2'/>
        <Send   
